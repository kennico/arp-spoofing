...
namespace kni {
    ...
    class hijack_http : public io_packet_buf, public tcp_packet {
    public:

        hijack_http(const lan_info *l, const endpoint_t &remote, port_t t = 80) :
        // io_packet_buf(snap_len) Passing an uninitialized(or 0?) variable in base constructor is dangerous and difficult to debug
                io_packet_buf(SNAP_LEN), tcp_packet(),
                target(t), lan(l), httpd(remote), port_manager(10032, 0x10000, 60 * 60) {

            KNI_LOG_DEBUG("httpd %s:%d", kni::to_string(httpd.ip).c_str(), httpd.port);
            for (auto &p : lan->mapping())
                KNI_LOG_DEBUG("IPv4:%s - %s", p.first.c_str(), kni::to_string(p.second).c_str());
        }
    ...
    protected:
        ...
        inline void send_fake_packet(const endpoint_t &from, const endpoint_t &to) {
            ...
            if (!send_packet(send_buf.get(), cap_info.caplen))
                ...
                KNI_LOG_DEBUG("Forward %s:%d -> %s:%d",
                              kni::to_string(sender.ip).c_str(), sender.port,
                              kni::to_string(receiver.ip).c_str(), receiver.port);
                KNI_LOG_DEBUG("Masquerade %s:%d -> %s:%d",
                              kni::to_string(from.ip).c_str(), from.port,
                              kni::to_string(to.ip).c_str(), to.port);
            }
    ...
    public:
        void handle_packet() override {
            ...
            KNI_LOG_DEBUG("Packet %s:%d -> %s:%d",
                          kni::to_string(sender.ip).c_str(), sender.port,
                          kni::to_string(receiver.ip).c_str(), receiver.port);

            if (victims.count(sender.ip) == 1 && receiver.port == target)
